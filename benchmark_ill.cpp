...
class Projectile {
	public:
		...
		idLinkList<Projectile> allItems;
		idLinkList<Projectile> renderables;
		idLinkList<Projectile> projectiles;
...
Projectile::Projectile(float x, float y, float z, float vel_x, float vel_y, float vel_z): x(x), y(y), z(z), vel_x(vel_x), vel_y(vel_y), vel_z(vel_z) {
	allItems = idLinkList<Projectile>();
	allItems.SetOwner(this);

	renderables = idLinkList<Projectile>();
	renderables.SetOwner(this);

	projectiles = idLinkList<Projectile>();
	projectiles.SetOwner(this);
}
...
int main() {
	auto *allItemsHead = new idLinkList<Projectile>();
	auto *renderablesHead = new idLinkList<Projectile>();
	auto *projectilesHead = new idLinkList<Projectile>();
...

	{
		...
		projectile->allItems.InsertAfter(*allItemsHead);
		projectile->renderables.InsertAfter(*renderablesHead);
		projectile->projectiles.InsertAfter(*projectilesHead);
	}
	int projectilesGenerated = 1;

	while (!projectilesHead->IsListEmpty()) {
		auto renderLoopStart = clock();

		auto elapsedSec = (renderLoopStart - lastLoopTime) / float(CLOCKS_PER_SEC);
		auto sinceStart = difftime(renderLoopStart, start);
		...
		auto phyItem = projectilesHead->Next();
		while (phyItem != NULL) {
			phyItem->ApplyGravity(elapsedSec);
			phyItem->ApplyVelocity(elapsedSec);

			auto nextItem = phyItem->projectiles.Next();
		...
		auto rendItem = renderablesHead->Next();
		while (rendItem != NULL) {
			rendItem->Render();
			rendItem = rendItem->renderables.Next();
		}
	...
	// TODO: Break this out into a separate library, and use in both benchmarks. Also, add standard deviation and a bit of histogram output, if possible.
